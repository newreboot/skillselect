...
class GameWindow(ui.ScriptWindow):
	...
	def Open(self):
		...
		if debugInfo.IsDebugMode() or constInfo2.IsDebugMode():
	...
	def __BuildKeyDict(self):
		...
		if app.ENABLE_DETAILS_UI:
			onPressKeyDict[app.DIK_F7] = lambda: self.chDetailsWnd.Show()
		...
		onPressKeyDict[app.DIK_C] = self.__PressCKey
	...
	if app.ENABLE_EXPRESSING_EMOTION:
		def ClearSpecialEmotions(self):
			constInfo2.SPECIAL_EMOTIONS = []
			constInfo2.SPECIAL_EMOTIONS_TIME = []

			self.interface.ClearSpecialEmotions()

		def RegisterSpecialEmotion(self, emotionIdx, leftTime):
			SPECIAL_EMOTIONS = constInfo2.SPECIAL_EMOTIONS
			SPECIAL_EMOTIONS_TIME = constInfo2.SPECIAL_EMOTIONS_TIME
			...
			if not emotionIdx in SPECIAL_EMOTIONS:
				if len(SPECIAL_EMOTIONS) < 12:
					...
					chat.AppendChat(chat.CHAT_TYPE_INFO, localeInfo.OPEN_THE_SPECIAL_ACTION_KIT % constInfo2.GET_SPECIAL_EMOTION_NAME(emotionIdx))

	if app.ENABLE_CAPTCHA_SYSTEM:
		def IsAFKMode(self, qid):
			constInfo2.AFKMode = int(qid)
	...
	def ShowConsole(self):
		if debugInfo.IsDebugMode() or True == self.consoleEnable or constInfo2.IsDebugMode():
				...
				if player.IsEquipmentSlot(attachedItemSlotPos) and player.SLOT_TYPE_DRAGON_SOUL_INVENTORY != attachedType:
					...
					if chr.IsNPC(dstChrID):
						if app.ENABLE_AUTO_REFINE:
							constInfo2.AUTO_REFINE_TYPE = 2
							constInfo2.AUTO_REFINE_DATA["NPC"][0] = dstChrID
							constInfo2.AUTO_REFINE_DATA["NPC"][1] = attachedInvenType
							constInfo2.AUTO_REFINE_DATA["NPC"][2] = attachedItemSlotPos
							constInfo2.AUTO_REFINE_DATA["NPC"][3] = attachedItemCount
	...
	if app.ENABLE_SEND_TARGET_INFO:
		def BINARY_AddTargetMonsterDropInfo(self, raceNum, itemVnum, itemCount):
			if not raceNum in constInfo2.MONSTER_INFO_DATA:
				constInfo2.MONSTER_INFO_DATA.update({raceNum : {}})
				constInfo2.MONSTER_INFO_DATA[raceNum].update({"items" : []})
			curList = constInfo2.MONSTER_INFO_DATA[raceNum]["items"]
	...
	if app.ENABLE_DUNGEON_INFO_SYSTEM:
		def DungeonInfo(self, questindex):
			constInfo2.dungeonData["quest_index"] = questindex

		def CleanDungeonInfo(self):
			constInfo2.dungeonInfo = []

		def CleanDungeonRanking(self):
			constInfo2.dungeonRanking["ranking_list"] = []
			constInfo2.dungeonRanking["my_ranking"] = []

		def GetDungeonInfo(self, cmd):
			cmd = cmd.split("#")

			if cmd[0] == "INPUT":
				constInfo2.INPUT_IGNORE = int(cmd[1])

			elif cmd[0] == "CMD":
				net.SendQuestInputStringPacket(constInfo2.dungeonData["quest_cmd"])
				constInfo2.dungeonData["quest_cmd"] = ""
		...
		def UpdateDungeonRanking(self, name, level, rankType):
			...
			constInfo2.dungeonRanking["ranking_list"].append([name, level, rankType],)
		...
		def UpdateMyDungeonRanking(self, position, name, level, rankType):
			...
			constInfo2.dungeonRanking["my_ranking"].append([position, name, level, rankType],)
	...
	if app.ENABLE_GROWTH_PET_SYSTEM:
		def SetGrowthPetSkillInfo(self, pet_id, skill_count, pet_skill1, pet_skill_level1, pet_skill2, pet_skill_level2, pet_skill3, pet_skill_level3):
			if int(pet_id) in constInfo2.THE_PET_SKILL_INFO:
				del constInfo2.THE_PET_SKILL_INFO[int(pet_id)]

			petSkillData = {
				int(pet_id) : {
					"skill_count" : int(skill_count),
					"pet_skill1" : int(pet_skill1),
					"pet_skill_level1" : int(pet_skill_level1),
					"pet_skill2" : int(pet_skill2),
					"pet_skill_level2" : int(pet_skill_level2),
					"pet_skill3" : int(pet_skill3),
					"pet_skill_level3" : int(pet_skill_level3),
				},
			}
			constInfo2.THE_PET_SKILL_INFO.update(petSkillData)

			self.PetAffectShowerRefresh()

		def SetGrowthPetInfo(self, pet_id, pet_level, evol_level, birthday, pet_nick, pet_hp, pet_def, pet_sp):
			if int(pet_id) in constInfo2.THE_PET_INFO:
				del constInfo2.THE_PET_INFO[int(pet_id)]

			petData = {
				int(pet_id) : {
					"pet_level" : int(pet_level),
					"evol_level" : int(evol_level),
					"birthday" : int(birthday),
					"pet_nick" : str(pet_nick),
					"pet_hp" : str(pet_hp),
					"pet_def" : str(pet_def),
					"pet_sp" : str(pet_sp)
				},
			}
			constInfo2.THE_PET_INFO.update(petData)

		def SetGrowthPetIcon(self, id, vnum):
			player.SetActivePetItemID(int(id))
			player.SetActivePetItemVNum(int(vnum))

			self.PetAffectShowerRefresh()

		def SetGrowthPetNick(self, name):
			self.interface.SetGrowthPetNick(name)

		def SetGrowthPetEvol(self, evol):
			self.interface.SetGrowthPetEvol(evol)

		def SetGrowthPetLevel(self, level):
			self.interface.SetGrowthPetLevel(level)

		def SetGrowthPetExp(self, exp, iexp, texp):
			constInfo2.GROWTH_PET_INFO["EXP"].append([exp, iexp, texp])
			self.interface.SetGrowthPetExp(exp, iexp, texp)

		def SetGrowthPetDuration(self, dur, tdur, age):
			constInfo2.GROWTH_PET_INFO["LIFETIME"].append([dur, tdur, age])
			self.interface.SetGrowthPetDuration(dur, tdur, age)

		def SetGrowthPetBonus(self, pet_hp, pet_def, pet_sp):
			constInfo2.GROWTH_PET_INFO["BONUS"].append([str(pet_hp), str(pet_def), str(pet_sp)])
			self.interface.SetGrowthPetBonus(str(pet_hp), str(pet_def), str(pet_sp))

		def SetGrowthPetType(self, pet_type):
			constInfo2.GROWTH_PET_INFO["TYPE"] = pet_type
			self.interface.SetGrowthPetType(int(pet_type))

		def SetGrowthPetSkill(self, slot, idx, level):
			self.interface.SetGrowthPetSkill(int(slot), int(idx), int(level))

		def GrowthPetUnsummon(self):
			player.ClearActivePetItem()
			self.interface.ClearGrowthPetStatus()

		def PetSkillUpgradeDlgOpen(self, slot, index, gold):
			self.interface.PetSkillUpgradeDlgOpen(int(slot), int(index), int(gold))
	...
	if app.ENABLE_HIDE_COSTUME_SYSTEM:
		def SetBodyCostumeHidden(self, hidden):
			constInfo2.HIDDEN_BODY_COSTUME = int(hidden)
			self.interface.RefreshVisibleCostume()

		def SetHairCostumeHidden(self, hidden):
			constInfo2.HIDDEN_HAIR_COSTUME = int(hidden)
			self.interface.RefreshVisibleCostume()

		def SetAcceCostumeHidden(self, hidden):
			if app.ENABLE_ACCE_COSTUME_SYSTEM:
				constInfo2.HIDDEN_ACCE_COSTUME = int(hidden)
				self.interface.RefreshVisibleCostume()
			else:
				pass
		...
		def SetWeaponCostumeHidden(self, hidden):
			if app.ENABLE_WEAPON_COSTUME_SYSTEM:
				constInfo2.HIDDEN_WEAPON_COSTUME = int(hidden)
				self.interface.RefreshVisibleCostume()
			else:
				pass
	...
	def __ServerCommand_Build(self):
		...
		if app.ENABLE_GROWTH_PET_SYSTEM:
			serverCommandList.update({
				"SetGrowthPetSkillInfo" : self.SetGrowthPetSkillInfo,
				"SetGrowthPetInfo" : self.SetGrowthPetInfo,
				"PetIcon" : self.SetGrowthPetIcon,
				"PetName" : self.SetGrowthPetNick,
				"PetEvolution" : self.SetGrowthPetEvol,
				"PetLevel" : self.SetGrowthPetLevel,
				"PetExp" : self.SetGrowthPetExp,
				"PetDuration" : self.SetGrowthPetDuration,
				"PetBonus" : self.SetGrowthPetBonus,
				"PetType" : self.SetGrowthPetType,
				"PetSkill" : self.SetGrowthPetSkill,
				"PetUnsummon" : self.GrowthPetUnsummon,
				"PetSkillUpgradeDlgOpen" : self.PetSkillUpgradeDlgOpen,
			})
		...
		if app.ENABLE_MAILBOX_SYSTEM:
			serverCommandList.update({
				"SERVER_MAILBOX_MAP" : self.SERVER_MAILBOX_MAP
			})
	...
	if app.ENABLE_OFFLINE_SHOP_SYSTEM:
		...
		def BINARY_OfflineShop_Sales(self, buyerName, itemVnum, itemCount, itemPrice, itemCheque, itemDate):
			constInfo2.OFFLINE_SHOP_SALES.append([buyerName, itemVnum, itemCount, itemPrice, itemCheque, itemDate],)
	...
	if app.ENABLE_MINI_GAME_OKEY:
		...
		def BINARY_Cards_ShowIcon(self, active):
			...
			if IsRumiGameEvent > 0:
				constInfo2.RUMI_GAME_EVENT = True
			else:
				constInfo2.RUMI_GAME_EVENT = False

			if constInfo2.EVENT_BANNER == True:
